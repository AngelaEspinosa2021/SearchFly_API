...
namespace SearchFly_API.Repository
{
    public class FlightRepository : IFlightRepository
    {
        ...
        public async Task<List<FlightDto>> SearchFlights(string departureStation, string arrivalStation, DateTime departureDate)
        {
            ...
            foreach( var flight in listFlights)
            {
                var List = from m in listFlights
                           where m.DepartureStation.ToLower() == departureStation.ToLower() && m.ArrivalStation.ToLower() == arrivalStation.ToLower() && (m.DepartureDate.Date == departureDate.Date)
                           select m;

                if(List.Count() > 0)
                {
                    listTemp = List.ToList();
                }
                else
                {
                    return null;
            ...
            {
                filterList.Add(list.DepartureDate);
            }

            shedulesAvailable = createShedulesAvailableList(departureStation, arrivalStation, departureDate, filterList);
            
            if(shedulesAvailable == null)
            {
                return null;
            }
            else
        ...
        private List<DateTime> createShedulesAvailableList(string departureStation, string arrivalStation, DateTime departureDate , List<DateTime> FilterList)
        {
